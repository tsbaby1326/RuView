...
- **WiFi-CSI pose: efficiency frontier + per-room calibration service** (ADR-150 §3.2–3.6). Two beyond-SOTA results on the MM-Fi benchmark, plus the deployment mechanism that resolves real-world generalization:
  ...
  - **Calibration service** — Python reference impl (`aether-arena/calibration/`: `calibrate.py` fits an adapter from a labeled clip, `infer.py` runs base + adapter; verified 3.09%→74.29% on an unseen MM-Fi room) **integrated into the Rust product pose engine** (`cog-pose-estimation`): `InferenceEngine::with_adapter()` + `cog-pose-estimation run --config <cfg> --adapter <room.safetensors>` apply architecture-agnostic LoRA deltas on the pose head at inference. 6/6 crate tests pass (incl. an adapter-changes-output integration test).
