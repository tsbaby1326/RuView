...
// WASM interface for our O(log n) solver
interface WasmSublinearSolver {
  solve_sublinear(matrix_json: string, b: Float64Array): any;
  page_rank_sublinear(adjacency_json: string, damping: number, personalized?: Float64Array): any;
  free(): void;
}

interface WasmModule {
  WasmSublinearSolver: {
    new(jl_distortion: number, series_truncation: number): WasmSublinearSolver;
  };
}

export class WasmSublinearSolverTools {
  private wasmModule: WasmModule | null = null;
  private solver: WasmSublinearSolver | null = null;
   ...
   */
  async pageRankSublinear(adjacency: number[][], damping: number = 0.85, personalized?: number[]): Promise<any> {
    if (!this.solver) {
