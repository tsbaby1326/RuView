...
export class PsychoSymbolicTools {
  ...
  async handleToolCall(name: string, args: any): Promise<any> {
    switch (name) {
      case 'psycho_symbolic_reason':
        return this.performDeepReasoningWithCache(
          args.query,
          args.context || {},
          args.depth || 5,
          args.use_cache !== false,
          args.cache_priority || 'normal'
        );
  ...
  private async performDeepReasoningWithCache(
    query: string,
    context: any,
    maxDepth: number,
  ...
  private getCacheStatus(detailed: boolean = false): any {
  ...
  private clearCache(confirm: boolean = false): any {
    if (!confirm) {
      return {
  ...
  private generateDomainInsights(query: string, patterns: string[], context: any): string[] {
  ...
  private applyContextualReasoning(query: string, context: any, patterns: string[]): string[] {
    const insights: string[] = [];
  ...
  private analyzeEdgeCases(query: string, concepts: string[]): string[] {
  ...
  private extractEntitiesAndConcepts(query: string) {
    ...
    if (queryLower.includes('edge case')) concepts.push('edge_cases');
    if (queryLower.includes('hidden')) concepts.push('hidden_complexity');
    if (queryLower.includes('api')) concepts.push('api_design');
    if (queryLower.includes('user')) concepts.push('user_management');
  ...
  private generateDefaultInsights(query: string, patterns: string[], context: any): string[] {
    const insights: string[] = [];
    const queryLower = query.toLowerCase();
