...
export class OptimizedSolverTools {
   ...
   */
  private static createCSRMatrix(triplets: [number, number, number][], rows: number, cols: number) {
   ...
   */
  private static multiplyCSR(matrix: any, x: number[], y: Float64Array): void {
    y.fill(0);
   ...
   */
  private static denseToCSR(matrix: any): any {
  ...
  static async solve(params: any) {
    ...
    try {
      ...
      const solution = this.conjugateGradient(
        csrMatrix,
        params.vector,
   ...
   */
  private static async fallbackSolve(params: any) {
   ...
   */
  static async estimateEntry(params: any) {
   ...
   */
  static async batchSolve(matrix: any, vectors: number[][], params: any = {}) {
    const results: any[] = [];
    let totalTime = 0;
